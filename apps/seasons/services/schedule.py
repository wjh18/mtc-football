...
def generate_non_conf_matchups(league_structure):
    ...
    div_pairs = [
        [t1.teams.all(), t2.teams.all()] for t1, t2 in zip(conf1_divs, conf2_divs)
    ]
    ...
    for team in teams:
        for week in range(5, 13):
            for i in range(4):
                ...
                rows.append(
                    ([team_in_week[team][week], team_bye[team], week_bye[week][i]], tag)
                )
